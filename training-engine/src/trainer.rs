...
/// Minimum z for a result to be considered a real win (not noise).
/// Used both for futility-stop (project z to max_games) and final accept criterion.
...
pub struct EvalResult {
    ...
    // Football points (3=win, 1=draw, 0=loss)
    pub wins: u64,
    pub draws: u64,
    pub losses: u64,
...
struct GameTotals {
    ...
    tackle_success: u64,
    out_of_bounds: u64,
    // Points tracking (3=win, 1=draw, 0=loss)
    wins: u64,    // candidate wins
    draws: u64,
    losses: u64,  // baseline wins
}
...
fn run_one_team_v4_game(baseline: &TeamPolicyV4, candidate: &TeamPolicyV4, seed: u64, swap: bool) -> (u32, u32, crate::game::Stats) {
    let mut rng = SmallRng::seed_from_u64(seed);
    let (t0, t1) = if swap { (candidate, baseline) } else { (baseline, candidate) };
    let mut game = Game::for_team_battle_v4(t0, t1);
    while game.phase != Phase::Fulltime {
        step_game(&mut game, &mut rng);
    }
...
/// v4 evaluation: same shape as v3 but uses V4Params.
pub fn evaluate_team_policies_v4(
    baseline: &TeamPolicyV4,
    candidate: &TeamPolicyV4,
    max_games: usize,
) -> EvalResult {
    ...
    while games_run < max_games {
        ...
        let chunk_results: Vec<(u32, u32, crate::game::Stats)> = seeds
            .into_par_iter()
            .map(|(seed, swap)| run_one_team_v4_game(baseline, candidate, seed, swap))
            .collect();
    ...
    EvalResult {
        ...
        candidate_points: 0.0, baseline_points: 0.0,
        point_diff: 0.0, point_z_score: 0.0,
    }
}
...
fn run_one_team_v6_game(baseline: &TeamPolicyV6, candidate: &TeamPolicyV6, seed: u64, swap: bool) -> (u32, u32, crate::game::Stats) {
    let mut rng = SmallRng::seed_from_u64(seed);
    let (t0, t1) = if swap { (candidate, baseline) } else { (baseline, candidate) };
    let mut game = Game::for_team_battle_v6(t0, t1);
...
pub fn evaluate_team_policies_v6(
    ...
    while games_run < max_games {
        ...
        for (b_goals, c_goals, stats) in chunk_results {
            ...
            totals.out_of_bounds += stats.out_of_bounds as u64;
            diffs.push(c_goals as f64 - b_goals as f64);
            // Points per game: 3=win, 1=draw, 0=loss (from candidate's perspective)
            let (pd, outcome) = if c_goals > b_goals {
                (3.0_f64 - 0.0, 1u8) // candidate wins
            } else if c_goals == b_goals {
                (1.0 - 1.0, 0u8)     // draw → net 0
            } else {
                (0.0 - 3.0, 2u8)     // baseline wins
            };
            point_diffs.push(pd);
        ...
        games_run += chunk_size;
        if games_run >= WINDOW_MIN_GAMES {
            // POINTS-BASED early stop: use point_diffs z-score (3=win, 0=draw, -3=loss)
            // This makes training optimize for win-rate, not goal-margin.
    ...
    EvalResult {
        ...
        candidate_avg_goals: (totals.candidate_goals as f64 / n * 1000.0).round() / 1000.0,
        goal_diff: (goal_diff * 1000.0).round() / 1000.0,
        // POINTS-BASED accept criterion: candidate wins if point z-score significant
