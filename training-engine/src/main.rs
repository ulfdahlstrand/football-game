...
fn main() {
    ...
    if args.get(1).map(|s| s.as_str()) == Some("--v7-team-test") {
        // Usage: --v7-team-test <team> [matches]
        let team_name = args.get(2).cloned().unwrap_or_else(|| "aurora-fc".to_string());
        let n_matches: usize = args.get(3).and_then(|s| s.parse().ok()).unwrap_or(100);
        run_v7_team_test(&project_root, &team_name, n_matches);
        return;
    }

    if args.get(1).map(|s| s.as_str()) == Some("--v7-tournament") {
        let games_per_match: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(1000);
        run_v7_tournament(&project_root, games_per_match);
        return;
    }
...
fn run_v7_team_test(project_root: &Path, team_name: &str, n_matches: usize) {
    ...
    use crate::session::{load_v7_team, read_team_baseline_v6};
    ...
    for seed in 0..n_matches as u64 {
        ...
        // Varannan match: V7 från respektive sida för att neutralisera side-bias
        let swap = seed % 2 == 1;
...
fn run_v7_tournament(project_root: &Path, games_per_match: usize) {
    use rand::SeedableRng;
    use rand::rngs::SmallRng;
    use crate::game::{Game, Phase};
    use crate::physics::step_game;
    use crate::team::Team;
    use crate::session::load_v7_team;
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            // Poäng: 3/1/0 baserat på totalt målantal
